...
public class GraphAlgorithms {
    ...
    public static <T> Set<Edge<T>> prims(Vertex<T> start, Graph<T> graph) {
        ...
        Map<Vertex<T>, List<VertexDistance<T>>> newMap = graph.getAdjList();

        for (VertexDistance<T> n : newMap.get(start)) {
            pq.add(new Edge<>(start, n.getVertex(), n.getDistance()));
        }
        ...
        while (!pq.isEmpty() && !(visited.size() == graph.getVertices().size())) {
            ...
            if (!visited.contains(currEdge.getV())) {
                ...
                mst.add(new Edge<>(currEdge.getU(), currEdge.getV(), currEdge.getWeight()));
                mst.add(new Edge<>(currEdge.getV(), currEdge.getU(), currEdge.getWeight()));

                for (VertexDistance<T> n : newMap.get(currEdge.getV())) {
                    if (!visited.contains(n.getVertex())) {
                        pq.add(new Edge<>(currEdge.getV(), n.getVertex(), n.getDistance()));
                    }
                }
            }
        }
