...
public class ExternalChainingHashMap<K, V> {
    ...
    private void resizeBackingTable(int length) {
        ...
        for(ExternalChainingMapEntry<K, V> n : oldTable) {
            while(n != null) {
                int index = Math.abs(n.getKey().hashCode() % table.length);
                ExternalChainingMapEntry<K, V> head = table[index];
                while(head != null) {
                    if(head.getKey().equals(n.getKey())) {
                        head.setValue(n.getValue());
                    }
                    head = head.getNext();
                }
                head = table[index];
                ExternalChainingMapEntry<K, V> node = new ExternalChainingMapEntry<>(n.getKey(), n.getValue());
                node.setNext(head);
                table[index] = node;
                size++;
                n = n.getNext();
