...
public class AVL<T extends Comparable<? super T>> {
    ...
    public T remove(T data) {
        ...
        AVLNode<T> delete = new AVLNode<>(null);
        root = removeHelper(root, data, delete);
        return delete.getData();
    }
    private AVLNode<T> removeHelper(AVLNode<T> node, T data, AVLNode<T> delete) {
        if(node == null) {
            throw new NoSuchElementException("error");
        }
        int temp = data.compareTo(node.getData());
        if(temp < 0) {
            node.setLeft(removeHelper(node.getLeft(), data, delete));
        } else if(temp > 0) {
            node.setRight(removeHelper(node.getRight(), data, delete));
        } else {
            size--;
            delete.setData(node.getData());
            if(node.getLeft() == null) {
                return node.getRight();
            } else if(node.getRight() == null) {
                return node.getLeft();
            } else {
                AVLNode<T> childNode = new AVLNode<>(null);
                node.setRight(successor(node.getRight(), childNode));
    ...
    private AVLNode<T> successor(AVLNode<T> node, AVLNode<T> childNode) {
        if(node.getLeft() == null) {
            childNode.setData(node.getData());
            return node.getRight();
        }
        node.setLeft(successor(node.getLeft(), childNode));
        updateHeightAndBF(node);
        return balance(node);
    }
    ...
    private void updateHeightAndBF(AVLNode<T> node) {
        ...
        int updateH = Math.max(leftH, rightH) + 1;
        int updateBF = leftH - rightH;

        node.setHeight(updateH);
        node.setBalanceFactor(updateBF);
    }
    ...
    private AVLNode<T> rotateLeft(AVLNode<T> currentNode) {
        // WRITE YOUR CODE HERE (DO NOT MODIFY METHOD HEADER)!
        AVLNode<T> root = currentNode.getRight();
    ...
    private AVLNode<T> rotateRight(AVLNode<T> currentNode) {
        // WRITE YOUR CODE HERE (DO NOT MODIFY METHOD HEADER)!
        AVLNode<T> root = currentNode.getLeft();
